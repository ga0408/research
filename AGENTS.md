...
5. **서술형 학술 문단 작성 및 시각 자료(Figure) 임베딩 규칙**:
   ...
   - **수식 및 특수문자 표기 규칙 (Strictly No Raw LaTeX / Clean Unicode & Code Blocks)**:
     - **LaTeX 문법 사용 전면 금지**: 깃허브(GitHub) 웹 렌더러, 터미널, 모바일, 온프레미스 Git 등 다양한 환경에서의 직관적인 가독성을 위해 raw LaTeX 구문(`$ ... $`, `$$ ... $$`, `\theta`, `\frac`, `\mathcal`, `\sum` 등)을 **절대 사용하지 않는다**.
