...
5. **서술형 학술 문단 작성 및 시각 자료(Figure) 임베딩 규칙**:
   ...
   - **수식 및 특수문자 표기 규칙 (No Raw LaTeX / Clean Unicode & Code Blocks)**:
     - 깃허브(GitHub) 마크다운 뷰어, 터미널, 모바일 등 다양한 환경에서의 직관적인 가독성을 위해 raw LaTeX 구문(`$ ... $`, `$$ ... $$`, `\theta`, `\frac`, `\mathcal`, `\sum` 등)을 사용하지 않는다.
     - 인라인 변수 및 수식은 직관적인 유니코드 수학 기호(`θ`, `Σ`, `α`, `λ`, `δ`, `Δ`, `κ`, `∈`, `∉`, `≤`, `≥`, `≠`, `≈`, `→`, `←`, `⇒`, `⟨`, `⟩`, `∑`, `∏`, `×`, `·`, `²`, `³`)와 백틱(`` `θ` ``, `` `L(θ)` ``, `` `PPL_aug` ``, `` `O(1)` ``)을 사용하여 표기한다.
     - 다중 라인 수식은 읽기 쉬운 코드 블록(```)으로 서식화한다.
     - 금액 표기 등 일반 텍스트의 달러 표시는 수식 오파싱을 방지하기 위해 `\$`로 이스케이프하거나 `USD`/`달러`로 작성한다.
