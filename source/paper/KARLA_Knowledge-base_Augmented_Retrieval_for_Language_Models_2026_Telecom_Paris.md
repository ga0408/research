...
> "We are given a pre-trained language model and a knowledge base (KB). Formally, a KB is a set of triples of the form `⟨s, r, o⟩`, where `s` is a subject entity, `r` is a relation (or predicate) from a closed set `R`, and `o` is the corresponding object, as in `⟨Paris, populationTotal, 2,047,602⟩`. Our objective is to fine-tune the model so that it interleaves natural-language generation with inline queries of the form `⟨s, r, ?⟩`. These inline queries are then replaced by an object `o` for which `⟨s, r, o⟩` is in the KB." (§3.1)
...
> "In practice, the inline query `⟨s, r, ?⟩` with result `o` is expressed as:
> ```
> ⟨r⟩⟨subj⟩s⟨/subj⟩⟨KB⟩o : o_desc⟨/KB⟩
> ```
> In this sequence, `⟨r⟩` is a relation-specific trigger token. The tag `⟨subj⟩` marks the subject, which is used to query the KB. The model will be trained to identify the subject here from the preceding part of the sentence. The tag `⟨KB⟩` marks the answer to the query that was retrieved from the KB. It is included in the output sequence of tokens as soon as the inline query is generated, so as to allow the model to condition the following tokens on `o`. The text `o_desc` is a short description of the object as retrieved from the KB (e.g., by help of the relation schema:description)." (§3.1)

> "We represent each relation as an atomic special token `⟨r⟩` rather than as a plain-text label. This has the advantage that the model cannot generate relations that do not exist in the KB. It also reduces per-call inference cost, because each retrieval action emits one token rather than the several sub-tokens of a plain-text predicate label. To speed up convergence, we initialize the new embeddings from the base tokenizer. Let `E ∈ ℝ^(|V| × d)` denote the embedding matrix of the original tokenizer where `V` is the vocabulary set and `d` is the embedding dimension. For a relation `r`, we define the embedding of the corresponding predicate token `E_⟨r⟩` as the mean of its constituent sub-token embeddings:
> ```
> E_⟨r⟩ = (1 / |t(r)|) * ∑_{w ∈ t(r)} E_w
> ```
> where `t(r)` is the sub-token decomposition of predicate label `r`." (§3.1)
...
> Input: Knowledge base `KB`, relation set `R`, per-relation budget `T`, relations per sample `k`.
> Output: Training set of entity subgraphs `S`.
> 1: counts[r] ← 0 for all r ∈ R
> 2: S ← ∅
> 3: while ∃ r ∈ R such that counts[r] < T do
> 4:   Sample entity e ~ Entities(KB) uniformly with replacement
> 5:   F(e) ← { ⟨e, r, o⟩ ∈ KB | counts[r] < 2T }
> 6:   F_valid(e) ← deduplicate(F(e))
> 7:   if F_valid(e) ≠ ∅ then
> 8:     F_sample ← sample min(k, |F_valid(e)|) facts from F_valid(e)
> 9:     S ← S ∪ {(e, F_sample)}
> 10:    for ⟨e, r, o⟩ ∈ F_sample do
> 11:      counts[r] ← counts[r] + 1
> 12: return S" (Appendix B)

> **Proposition 1 (Relation Budget Bounds)**:
> "Let `S` be the output of Algorithm 1. For every relation `r ∈ R`, the final count satisfies:
> ```
> T ≤ counts[r] < 2T
> ```
> Proof: Since `R` is constructed from the KB, every relation has at least one supporting triple, so `E(r) ≠ ∅` for all `r ∈ R`.
> Lower bound: The while loop terminates only when counts[r] ≥ T for all r ∈ R. Thus counts[r] ≥ T.
> Upper bound: Line 5 excludes any relation with counts[r] ≥ 2T from candidate selection. Since counts[r] is incremented by at most 1 per sample step, the condition counts[r] < 2T ensures that counts[r] cannot reach or exceed 2T before exclusion." (Appendix B)
