...
LLM에서 입력 텍스트가 훈련 시 지정된 컨텍스트 윈도우(Context Size)를 넘어서면 이전 대화나 정보를 잊어버리는 것과 마찬가지로, 자기회귀(Autoregressive) 비디오 월드 모델 역시 생성 프레임 길이가 훈련 컨텍스트 한계(`L_train`)를 넘어서면 과거 시각 관측을 상실하게 된다.

그러나 비디오 월드 모델에서는 단순한 메모리 용량 초과를 넘어 **두 가지 고유한 기술적 병목**이 결합되어 나타난다:
1. **RoPE 오프셋 이탈에 따른 Addressability 붕괴**: 프레임 생성이 길어질수록 쿼리와 과거 키 간의 상대 위치 거리(`δ = q - t`)가 훈련 범위를 벗어나, 과거 관측이 KV 캐시에 저장되어 있음에도 주의집중(Attention) 점수가 0으로 수렴하여 인출에 실패한다.
...
RoPE는 모델이 토큰(프레임)의 위치를 기억하도록 **벡터를 2차원 평면상에서 특정 각도(`θ × t`)만큼 시계 바늘처럼 회전시키는 기술**이다.
...
- **상대 위치(Relative Position) 계산**: 
  - 쿼리(현재 질문 시점 `q`)와 과거 키(답변 시점 `t`) 간의 Attention(주의집중) 점수를 계산할 때, 모델은 절대 시간 `t`가 아니라 두 시각 바늘의 **'각도 차이(상대 거리 `δ = q - t`)'**를 보고 정보를 읽어온다.
...
- **문제 발생 원인**:
  - 모델을 훈련시킬 때 비디오 길이는 최대 6개 청크(약 18 프레임) 수준이었으므로, 모델은 상대 오프셋 `δ ≤ 18` 범위 내의 각도 차이만 보고 과거를 읽도록 학습되었다.
  - 하지만 실시간 생성 길이가 길어져 `q = 100` 시점에 도달하면, `t = 0` 시점의 과거 메모리를 읽으려 할 때 오프셋이 `δ = 100`으로 커진다.
...
> - **전체 구조**: 전체 주의집중 윈도우 `L_attn`를 최근 윈도우 `R` (`N_r` 슬롯)과 먼 과거 요약 캐시 `S` (`N_s` 슬롯)로 분할.
...
+--------------------------------------------------------------------------------------------------+
  ...
  - 조건: 재방문/시점 anchor 없는 일방향 생성 (N = 48 Chunks)       - 조건: A → B → A, A → B → C → A 등
...
> LoopBench는 재방문 시점의 회상 성능을 정밀 평가하기 위한 전용 벤치마크 기하학 구조임 (`ABA × 8`, `ABCA × 5,5,7`, `ABCDA × 4,4,4,4`).
...
| 구분 | 적용 조건 | TempSSIM (↑, 일관성) | Local Scene Drift (↓, 붕괴율) | LoopBench Revisit PAC (↑, 회상률) |
| :--- | :--- | :---: | :---: | :---: |
