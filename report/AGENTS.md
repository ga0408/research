...
5. **수식 및 특수문자 표기 규칙 (Strictly No Raw LaTeX / Clean Unicode & Code Blocks)**:
   - **LaTeX 문법 사용 전면 금지**: 깃허브(GitHub) 웹 렌더러, 터미널, 모바일 등 다양한 환경에서의 직관적인 가독성을 위해 raw LaTeX 구문(`$ ... $`, `$$ ... $$`, `\theta`, `\frac`, `\mathcal`, `\sum` 등)을 **절대 사용하지 않는다**.
   - **인라인 변수 및 수식 표기**: 백틱(`` `...` ``)과 직관적인 유니코드 수학 기호 조합을 사용한다.
     - 그리스 문자: `θ`, `Σ`, `α`, `β`, `γ`, `δ`, `Δ`, `ε`, `λ`, `μ`, `π`, `σ`, `τ`, `φ`, `ψ`, `ω`, `κ`, `ξ`
     - 연산자·관계: `∈`, `∉`, `⊆`, `⊂`, `∪`, `∩`, `≤`, `≥`, `≠`, `≈`, `≡`, `±`, `×`, `÷`, `·`, `√`, `∞`
     - 화살표·논리: `→`, `←`, `⇒`, `⇔`, `∀`, `∃`, `¬`, `∧`, `∨`, `↑`, `↓`
     - 괄호·브래킷: `⟨`, `⟩`, `[`, `]`, `{`, `}`
     - 첨자·지수: `_t`, `_i`, `_{t+1}`, `²`, `³`, `^T`, `^-1`, `10^-5`
   - **다중 라인/블록 수식 표기**: LaTeX 블록(`$$...$$`) 대신 읽기 쉬운 **코드 블록(```)**으로 서식화한다.
   - **금액 및 일반 달러 기호**: 마크다운 파서가 수식으로 오인하는 것을 방지하기 위해 `\$`로 이스케이프하거나 `USD`/`달러`로 작성한다.
   - **변환 매핑 기준표**:
     | Raw LaTeX (사용 금지 ❌) | 클린 마크다운 표기 (필수 준수 ⭕) |
     |---|---|
     | `$\theta$`, `$\alpha$`, `$\lambda$`, `$\delta$` | `` `θ` ``, `` `α` ``, `` `λ` ``, `` `δ` `` |
     | `$\mathcal{L}(\theta)$`, `$\mathcal{R}$`, `$\mathcal{S}$` | `` `L(θ)` ``, `` `R` ``, `` `S` `` |
     | `$\sum_{t=1}^T$`, `$\prod_{i=1}^n$` | `∑_{t=1}^T`, `∏_{i=1}^n` |
     | `$\frac{a}{b}$` | `(a / b)` 또는 `a / b` |
     | `$\le$`, `$\ge$`, `$\approx$`, `$\neq$` | `≤`, `≥`, `≈`, `≠` |
     | `$\to$`, `$\leftarrow$`, `$\Rightarrow$` | `→`, `←`, `⇒` |
     | `$\langle s, r, o \rangle$` | `⟨s, r, o⟩` |
     | `$PPL_{aug}$`, `$O(1)$`, `$O(\text{facts})$` | `` `PPL_aug` ``, `` `O(1)` ``, `` `O(facts)` `` |
     | `$$\mathcal{L} = - \sum \dots$$` | 코드 블록(```) 내 텍스트 수식 |
     | `$10.5` (가격) | `\$10.5` 또는 `10.5달러` / `10.5 USD` |
6. **시각 자료(Figure 크롭 및 비유) 보강**: 비전/아키텍처 논문 분석 시 원본 PDF의 주요 Figure(실험 결과 비교, 메모리 구조도, 궤적 다이어그램 등)를 `source/paper/figures/`에 정밀 크롭하여 마크다운 이미지(`![caption](path)`)로 포함하며, 직관적인 개념 비유(예: 시계 바늘 회전)와 ASCII 다이어그램을 적극 활용한다.
7. **INDEX.md 등록 필수**: title, summary, keywords, topics 등 메타데이터를 INDEX.md에 등록.
8. **푸시 전 최종 검토 체크리스트 (Pre-Push Verification)**:
   - 푸시(`git push`) 직전에 에이전트는 아래 항목을 반드시 사전 점검(Check)한다:
     - [ ] `report/INDEX.md` 메타데이터 표 등록 완료 여부
     - [ ] 대화체/문답식 구문 없이 서술형 학술 문단으로 전개되었는가?
     - [ ] Raw LaTeX 수식(`$`, `\theta`, `\frac`, `\mathcal` 등) 없이 유니코드 및 코드 블록 수식으로 작성되었는가?
