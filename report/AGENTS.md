...
4. 핵심 알고리즘/구조는 세세하게 설명하되, **코드 원문보다 동작 의미·점수 공식·게이트 조건을 이해하기 쉽게** 풀어쓴다 (예: 가중치·임계값은 표로, 코드는 스니펫).
5. **INDEX.md 등록 필수**: title, summary, keywords, topics 등 메타데이터를 INDEX.md에 등록.
6. 분석에 인용한 핵심 코드/발췌는 `source/`에 저장하고 본문에서 상대경로로 링크. low-level 상세(검색 퓨전 공식, 추출 파이프라인, 점수화 등)는 본문이 아닌 스니펫에 분리한다.
