...
5. **수식 및 특수문자 표기 규칙 (No Raw LaTeX / Clean Unicode & Code Blocks)**:
   - 깃허브(GitHub) 웹 렌더러, 터미널, 모바일 등 다양한 환경에서의 직관적인 가독성을 위해 raw LaTeX 구문(`$ ... $`, `$$ ... $$`, `\theta`, `\frac`, `\mathcal`, `\sum` 등)의 사용을 지양한다.
   - 인라인 변수 및 수식은 직관적인 유니코드 수학 기호(`θ`, `Σ`, `α`, `λ`, `δ`, `Δ`, `κ`, `∈`, `∉`, `≤`, `≥`, `≠`, `≈`, `→`, `←`, `⇒`, `⟨`, `⟩`, `∑`, `∏`, `×`, `·`, `²`, `³`)와 백틱(`` `θ` ``, `` `L(θ)` ``, `` `PPL_aug` ``, `` `O(1)` ``)으로 표기한다.
   - 독립 블록 수식은 읽기 쉬운 코드 블록(```)으로 서식화한다.
   - 일반 텍스트의 달러 표시는 수식 오파싱 방지를 위해 `\$`로 이스케이프하거나 `USD`/`달러`로 작성한다.
...
8. **푸시 전 최종 검토 체크리스트 (Pre-Push Verification)**:
   - 푸시(`git push`) 직전에 에이전트는 아래 항목을 반드시 사전 점검(Check)한다:
     ...
     - [ ] Raw LaTeX 수식(`$`) 대신 유니코드 및 코드 블록 수식으로 깔끔하게 작성되었는가?
