...
### 핵심 메커니즘 요약 (동적 인라인 룩업 및 증강 생성 파이프라인)

```
[KARLA 동적 인라인 룩업 및 증강 생성 3단계 파이프라인]

1. 생성 중단 & 쿼리 방출 (LLM Generation)
   "Paris has a population of " ──► [ ⟨populationTotal⟩ ⟨subj⟩ Paris ⟨/subj⟩ ]
                                       ...
                                       (관계 특수 토큰)     (주어 엔티티 스팬)
                                              ...
                                              └────────┬────────┘
                                                       ▼
2. 런타임 인터셉트 & 외부 KB 조회 (KB Execution)
   Execution Engine이 쿼리 <Paris, populationTotal, ?> 조회 
   ──► 외부 DB/KB(YAGO, PrimeKG 등)에서 정답 인출: "2,047,602"
                                                       │
                                                       ▼
3. 사실 주입 & 후속 생성 재개 (Conditioned Generation)
   주입: [ ⟨KB⟩ 2,047,602 : capital of France ⟨/KB⟩ ]
   ──► 후속 생성: ", making it the most populous city in France."
```

- **동적 특수 토큰 쿼리 방출**: 모델이 자동회귀 문장 생성 도중 엔티티 관련 사실 진술이 필요한 위치에 도달하면, 단어를 임의로 찍어내지 않고 관계 특수 토큰과 주어 스팬(`⟨r⟩ ⟨subj⟩ s ⟨/subj⟩`)을 방출한다.
- **런타임 인터셉트 및 실시간 KB 인출**: 런타임 엔진이 쿼리 생성을 감지하여 일시 중지(Pause)하고, 외부 구조화 KB(YAGO, PrimeKG)에 `⟨s, r, ?⟩` 쿼리를 날려 정답 사실과 설명(`⟨KB⟩ o : o_desc ⟨/KB⟩`)을 인출한다.
- **컨텍스트 주입 및 후속 증강 생성**: 인출된 사실값을 컨텍스트에 즉시 주입하고, 모델은 주입된 사실을 조건부로 받아 자연스러운 후속 문장을 계속 생성한다.
- **그래디언트 차단 손실 마스킹 (`m_t = 0`)**: 외부 KB 반환값 토큰 구간의 손실을 0으로 마스킹하여 파라미터가 사실값을 암기하지 않고 오직 **"언제 어떤 쿼리를 호출할지"**만을 학습하도록 강제한다.

| 비교 항목 | 기존 Standard / Graph RAG | KARLA (동적 인라인 룩업 증강 생성) |
|---|---|---|
| **검색 시점** | 프롬프트 입력 시점에 1회성 일괄 검색·주입 | 문장 생성 도중 **사실이 필요한 정확한 타이밍**에 인라인 호출 |
| **파라미터의 역할** | 사실 암기 + 언어 표현이 가중치에 뒤엉킴 | **언어 표현 & 쿼리 생성** 전담, 사실값은 외부로 완전 오프로딩 |
| **모순 발생 시** | 내부 파라미터 기억으로 회귀(Parametric Override) | 사실을 외우지 않으므로 **외부 KB 값에 100% 의존·생성** |
| **지식 갱신 / 언러닝** | 파라미터 파인튜닝 필요 (비용 큼) | **외부 KB 레코드만 수정하면 0초 만에 즉각 갱신 (96.11%)** |
