...
- 모델의 초기 제로샷 성능($S_{\text{base}}$)과 피드백을 통한 개선 역량($\Delta_M$) 간에 **강한 양의 Pearson 상관관계(평균 $r = +0.50$)**가 확인되었다:
...
피드백을 소화하여 정답으로 전환시키는 개선 역량($\Delta_M$)이 모델의 기본 제로샷 추론 능력($S_{\text{base}}$)에 강하게 비례함을 8개 최신 LLM과 6개 벤치마크에 걸쳐 대규모로 정량화하였다. 즉, **"경험으로부터 스스로 개선되는 역량(Learning from Experience)"은 모델 기본 지능이 높을수록 가파르게 발현되는 창발적 특성(Emergent Property)**임을 입증하였다.

#### 4. 정답 전환의 2축(WHY / WHAT) 인과 분해
6,630개 오답→정답 전환 궤적을 인간-GPT 일치도($\kappa = 0.768$) 기반으로 분석하여, 전체 개선의 **47.7%가 피드백 충실도(Feedback Fidelity)**, **30.0%(코딩)가 문제 지문의 입출력 명세 재인식(Specification Recall)**에서 비롯됨을 밝혔다. 이는 모델이 단순히 무작위 탐색을 하는 것이 아니라 피드백과 문제 명세에 체계적으로 반응함을 보여준다.
