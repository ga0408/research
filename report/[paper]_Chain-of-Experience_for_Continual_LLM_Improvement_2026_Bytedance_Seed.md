...
최근 에이전트 메모리 연구(Dynamic CheatSheet, Agentic Context Engineering, SimpleMem 등)는 컨텍스트 윈도우 절약을 위해 과거 시도를 규칙이나 플레이북으로 '요약·압축'하여 주입하는 방식을 표준으로 삼아왔다. 그러나 본 논문의 통제 실험(Table 1, Table 3)은 **최신 고도 추론 모델(GPT-5, o3, Claude 4.5 Sonnet) 환경에서 과도한 메모리 압축이 중간 추론 단계의 핵심 논리 불변식(Invariant)과 엣지 케이스 세부조건을 파괴하여 성능을 심각하게 저하시킨다**는 사실을 입증하였다 (Claude 4.5 Sonnet AIME 기준: Model 단독 60.0% vs Model+DC 50.0%, Model+SimpleMem 56.7%). 즉, 단일 태스크 내에서는 요약 메모리보다 전체 궤적(Full Experience Trajectory)을 있는 그대로 컨텍스트에 보존하는 CoE가 월등히 우수함을 밝혔다.
